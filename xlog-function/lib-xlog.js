...
var XLog = function(logLevel, msgPrefix, msgTag, showDate, showTime, showMillitime, showMsgLevel, sides) {
  ...
  const _getParsedDate = function(date, separator, showDate, showTime, showMillitime) {
    ...
    var text = "";
    if(showDate) { text = now.getFullYear()+separator+pad(now.getMonth()+1)+separator+pad(now.getDate()); }
    if(showTime) { text += ""+pad(now.getHours())+separator+pad(now.getMinutes())+separator+pad(now.getSeconds()) }
    if(showMillitime) { text += ""+pad(now.getMilliseconds()) }
    return text;
  };
